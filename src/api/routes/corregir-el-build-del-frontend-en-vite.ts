...
router.get("/api/socios/:socio-id/cuotas", async (req: Request, res: Response) => {
  try {
    // TODO: implementar lógica de negocio
    res.status(200).json({ mensaje: 'OK' });
  } catch (_err) {
...
const patchApi_cuotas_cuota_id_pagarSchema = z.object({
  fecha_pago: z.string(),
});

router.patch("/api/cuotas/:cuota-id/pagar", async (req: Request, res: Response) => {
  try {
    const datos = patchApi_cuotas_cuota_id_pagarSchema.parse(req.body);
    // TODO: implementar lógica de negocio
